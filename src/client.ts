...
import { AgentArchiveParams, AgentCreateParams, AgentListParams, AgentListResponse, AgentResponse, AgentRetrieveParams, AgentUpdateParams, Agents, AmbientSound, AzureTranscriber, CaptureSettings, DeepgramTranscriber, Denoising, InactivitySettings, InitialMessage, InterruptSettings, ResponseTiming, Volume } from './resources/agents';
import { Account, AccountListConversationsParams, AccountListConversationsResponse, Accounts, ConversationDirection, ConversationStatus, ConversationType } from './resources/accounts/accounts';
...
export interface ClientOptions {
  /**
   * Defaults to process.env['ITELLICOAI_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['ITELLICOAI_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['ITELLICOAI_LOG'] or 'warn' if it isn't set.
   */
...
export class Itellicoai {
   ...
   */
  constructor({
    baseURL = readEnv('ITELLICOAI_BASE_URL'),
    apiKey = readEnv('ITELLICOAI_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.ItellicoaiError(
        'The ITELLICOAI_API_KEY environment variable is missing or empty; either provide it, or instantiate the Itellicoai client with an apiKey option, like new Itellicoai({ apiKey: \'My API Key\' }).'
      );
    }
    ...
    this.timeout = options.timeout ?? Itellicoai.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('ITELLICOAI_LOG'), 'process.env[\'ITELLICOAI_LOG\']', this) ?? defaultLogLevel;
  ...
  static Itellicoai = this;
  static DEFAULT_TIMEOUT = 60000 // 1 minute

  static ItellicoaiError = Errors.ItellicoaiError;
